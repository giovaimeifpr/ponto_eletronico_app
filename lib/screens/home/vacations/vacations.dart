  ...
  Widget _buildPeriodField(int index) {
    ...
    return Card(
      ...
      child: Column(
        children: [
          ListTile(
            // Só permite clicar se estiver editando
            onTap: _isEditing
                ? () {
                    VacationPickerDialog.show(
                      context: context,
                      index: index,
                      selectedPeriods: _selectedPeriods,
                      onSelected: (result) {
                        setState(() {
                          _selectedPeriods[index] = result;
                        });
                      },
                    );
                  }
            ...
            trailing: _isEditing && range != null
                ? IconButton(
                    icon: const Icon(Icons.close, color: Colors.redAccent),
                    onPressed: () {
                      setState(() {
                        _selectedPeriods[index] =
                            null; // Limpa a parcela específica
                      });
                    },
                  )
